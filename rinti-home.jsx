  ...
  return (
    <div className="trow">
      <span className="trow-ico">
        <Icon name="doc" size={20} stroke={1.7} />
      </span>
      ...
      <button className="trow-btn" onClick={() => onOpen(d)}>
        Abrir
      </button>
    </div>
  );
...
function ReadRow({ d, onRead }) {
  return (
    <div className="rrow">
      <span className="rrow-ico">
        <Icon name="doc" size={19} stroke={1.7} />
      </span>
      <span className="rrow-main">
        <span className="rrow-name">{d.name}</span>
        <code>{d.code}</code>
      </span>
      <button className="rrow-btn" onClick={() => onRead(d)}>
        Leer
      </button>
    </div>
  );
  ...
  return (
    <button className="short" style={{ "--tab": tab }} onClick={onClick}>
      ...
      <span className="short-panel">
        ...
        <span className="short-cta">
          Abrir <Icon name="arrow" size={15} stroke={2.3} />
        </span>
      </span>
    </button>
  );
...
function HomeScreen({ tabColor }) {
  ...
  const STATS = [
    ...
    },
    {
      ...
      go: () => fire("Abriendo pendientes de atención…"),
    },
    {
      id: "lec",
      icon: "read",
      ...
      value: "4",
      hint: "sin leer",
      go: () => fire("Abriendo lecturas…"),
    },
    {
      id: "venc",
      icon: "alert",
      tone: "coral",
      label: "Por vencer (4 años)",
      value: "16",
      hint: "próximos a renovar",
      go: () => fire("Abriendo documentos por vencer…"),
    },
  ];
  return (
    <div className="scroll">
          ...
          <div className="hero-content">
            <div className="hero-text">
              <h1 style={{
                ...
                Hola, María Quispe <span className="wave">👋</span>
              </h1>
              <p>
                Gestiona la elaboración, revisión y aprobación de tus documentos
            ...
            <div className="hero-cta">
              <button
                ...
                onClick={() => navigate("vigentes")}
              >
                <Icon name="eye" size={18} stroke={1.9} /> Consultar documentos
              </button>
              <button
                className="btn btn-bright"
                onClick={() => navigate("elaborar")}
              >
      ...
      <section className="cols">
        <div className="panel">
          <SectionHead
            ...
            sub="Documentos esperando tu intervención"
            action="Ver todas"
            onAction={() => fire("Abriendo Mis Tareas…")}
          />
          <div className="list">
            {ALL_DOCS.map((d) => (
              <TaskRow
        ...
        <div className="panel">
          <SectionHead
            title="Documentos pendientes de lectura"
            sub="Asignados que aún no han sido leídos"
            action="Ver todos"
            onAction={() => fire("Abriendo lecturas…")}
          />
          <div className="list">
            {LECTURA.map((d) => (
              <ReadRow
                key={d.code}
                d={d}
                onRead={(x) => fire("Marcando “" + x.name + "” como leído…")}
              />
            ))}
          </div>
        </div>
      ...
      >
        ...
        <div className="short-grid">
          <Shortcut
            icon="edit"
            tab={tabColor("var(--green)")}
            title="Crear documento"
            desc="Inicia la elaboración de un nuevo procedimiento, manual o especificación."
            onClick={() => navigate("elaborar")}
          />
          <Shortcut
            icon="tasks"
            tab={tabColor("var(--blue)")}
            title="Mis tareas"
            desc="Revisa, aprueba o lee los documentos asignados a ti."
            onClick={() => fire("Abriendo Mis Tareas…")}
          />
          <Shortcut
            icon="eye"
            tab={tabColor("var(--coral)")}
            title="Buscar y consultar"
            desc="Encuentra documentos vigentes filtrando por taxonomía, autor o estado."
            onClick={() => navigate("vigentes")}
          />
        </div>
      </section>
