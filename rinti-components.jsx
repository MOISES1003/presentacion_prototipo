  ...
  const item = (n) => (
    ...
    >
      <span className="nav-ico">
        <Icon name={n.icon} size={19} stroke={1.9} />
      </span>
      ...
      <div className="crumb">
        ...
        {crumb.map((c, i) => (
          <React.Fragment key={i}>
            ...
            <span className={i === crumb.length - 1 ? "crumb-here" : ""}>
              {c === "Taxonomía" ? "Configuración de Flujos" : c}
            </span>
          </React.Fragment>
        ))}
      </div>
      ...
      <span className="folder-panel">
        <span className="folder-top">
          <span className="folder-ico">
            <Icon name={icon} size={big ? 26 : 24} stroke={1.8} />
          </span>
...
function SectionHead({ title, sub, action, onAction }) {
  return (
    <div className="sec-head">
      ...
      {action && (
        <button className="sec-link" onClick={onAction}>
          {action}
  ...
  return (
    <Field label={label} required={required}>
      <div className="sel-wrap">
        <select
          className={"inp sel" + (value ? "" : " is-empty")}
          value={value}
          onChange={(e) => onChange && onChange(e.target.value)}
        >
  ...
  return (
    <div className="seg" style={{ "--n": options.length }}>
      <span
        className="seg-thumb"
        style={{
          transform: `translateX(${options.findIndex((o) => (o.value ?? o) === value) * 100}%)`,
        }}
      />
      {options.map((o) => {
        const v = o.value ?? o,
